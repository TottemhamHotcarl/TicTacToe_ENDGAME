...
def getChildern(root):
    ...
    if(pruneCheck(root.data)):

        DataX = partitionDataX(root.data, root.value)
        DataZero = partitionDataO(root.data, root.value)
        DataB = partitionDataB(root.data, root.value)
        # childO
        if len(DataZero) != 0 and root.whiteList is not None:
            temp = findBestGain(DataZero, root.whiteList)
            if temp[1] != -1:
                root.addChild(node(temp[1]), 'o', temp[1])
                root.childO.data = DataZero
                store[1] = temp[1]
...
for i in TestList:
    ...
    ConfusionMatrix[actualValue][guessValue] = ConfusionMatrix[actualValue][guessValue] + 1
